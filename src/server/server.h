 ...
 * Summary
 * -------
 * Header for server functions for `lxsrv`
 *
 * LICENSE
 * -------
...
typedef struct conn_t {
	...
	// Keep track of connections via this
	uint32_t connid;
...
typedef struct protocol_t {
	...
	void (*post)( server_t *, conn_t * );
} protocol_t;


//A log struct to handle working with a log stream.
typedef struct log {
	int (*open)( const char *, void **, char *, int );
	int (*close)( void * );
	int (*write)( void *, const server_t *, const conn_t * );
	void *data;
} log_t;
